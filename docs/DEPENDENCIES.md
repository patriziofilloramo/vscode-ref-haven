...
from reaching a packaged build.

## Direct development dependencies

| Package                                      | Purpose                                                                         |
| -------------------------------------------- | ------------------------------------------------------------------------------- |
| `typescript`, `@types/node`, `@types/vscode` | Strict compilation against a conservative Node API floor and VS Code 1.105 APIs |
...
- `engines.node` accepts the supported Node 22 LTS line from 22.13 and the
  Node 24 LTS line. Node 20 is no longer a supported development runtime.
- `@types/node` stays on Node 20 as a conservative API floor: production code
  must not accidentally depend on newer Node APIs merely because the build
  and release tools require a newer maintained runtime.
...
URL, privacy notice, security policy, and provenance record. The gate also
requires `private: true`: VSCE can publish the extension while npm itself
continues to reject an accidental `npm publish`.
